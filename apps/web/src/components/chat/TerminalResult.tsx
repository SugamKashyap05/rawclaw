...
export function TerminalResult({ result, framed = true }: { result: ToolResult; framed?: boolean }) {
  ...
  return (
    <ToolActivityCard sourceLabel="Terminal Result" status={result.error ? 'failed' : 'success'} durationMs={result.duration_ms}>
      {body}
    </ToolActivityCard>
  );
}
